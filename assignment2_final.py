...
KEY_PATH = os.path.join(BASE_DIR, ".", "halil_key.pem")
# ---------------------
...
class PoWCommunity(Community):
    ...
    def __init__(self, settings: CommunitySettings):
        ...
        self.add_message_handler(2, self.on_register_response)
        self.add_message_handler(10, self.on_ready)
        ...
        for peer in self.get_peers():
            if peer.public_key.key_to_bin() == public_key:
                ...
                if public_key == SERVER_PUBLIC_KEY:
                    print(f"Server peer found: {peer}")
                    self.server_peer = peer
                elif public_key == PEER1_PK:
                    print(f"Peer 1 found: {peer}")
                    self.peer1 = peer
                elif public_key == PEER2_PK:
                    print(f"Peer 2 found: {peer}")
                    self.peer2 = peer
    ...
    def on_challenge_response(self, peer: Peer, payload):
        ...
        print("Peer 1 is " + (str(self.peer1.public_key.key_to_bin()) if self.peer1 else "not found"))
        print("Peer 2 is " + (str(self.peer2.public_key.key_to_bin()) if self.peer2 else "not found"))
    ...
    def register_group(self):
        self.ez_send(self.server_peer, RegisterPayload(MY_PEER_PK, PEER1_PK, PEER2_PK))
    ...
    def on_register_response(self, peer: Peer, payload: RegisterResponsePayload) -> None:
        ...
        if payload.success:
            ...
            print("Group ID is: ", self.group_id)
    ...
    def on_receive_signed_nonce(self, peer: Peer, payload):
        ...
        if peer == self.peer1:
            ...
            if self.sig2 is not None:
                self.send_submission(self.server_peer, self.group_id, self.round, self.my_sig, self.sig1, self.sig2)
    ...
    def on_ready(self, peer: Peer, payload):
        if peer.public_key.key_to_bin() not in [PEER1_PK, PEER2_PK]:
            print(f"Ignoring response from unknown peer")
            return
        ...
        print(f"Received ready signal from peer {peer.public_key.key_to_bin()}")
...
async def start_client():
    ...
    while community.ready_count < 2:
        await asyncio.sleep(1.0)

    # First peer sends request
    community.send_challenge_request(server, community.group_id)
