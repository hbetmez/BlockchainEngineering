...
class NoncePayload(Payload):
    ...
    def __init__(self, nonce: bytes, group_id: str = ""):
        super().__init__()
        self.nonce = nonce
        self.group_id: str = group_id  # Set group_id when payload is created, since it's needed for signing

    def to_pack_list(self) -> list[tuple]:
        return [
            ("varlenH", self.nonce),
            ("varlenH", self.group_id),
        ]
...
class Group:
    def __init__(self) -> None:
        ...
        self.received_signatures = {}  # Maps peer position (1,2,3) to signature bytes
        self.expected_peers = []  # List of peer public keys, sorted
        self.peer_map = {}  # Maps public key bytes to Peer objects
        self.ready_peers = 0


class PoWCommunity(Community, PeerObserver):
    ...
    def __init__(self, settings: CommunitySettings):
        ...
        self.my_round = 0  # This peer's position (1, 2, or 3) - set after peer discovery
        self.group = Group()
    ...
    def on_peer_added(self, peer: Peer) -> None:
        ...
        if peer_key == SERVER_PUBLIC_KEY:
            ...
            print(f"Discovered Halil")
            self.peer1 = peer

        elif peer_key == PEER2_PK:
            print(f"Discovered Matei")
            self.peer2 = peer

        if self.server_peer and self.peer1 and self.peer2 and not self.is_started:
            self.is_started = True
            # print("TEST: sending challenge request to server")
    ...
    def on_peer_removed(self, peer: Peer) -> None:
        ...
        # if peer_key == SERVER_PUBLIC_KEY:
        #     self.server = peer
        #     print(f"[Peer {self.my_round}] Server connected")
        #     self.check_ready_to_send_challenge()
        #     return

        # # Handle client peer discovery
        # if peer_key not in self.group.expected_peers:
        #     self.group.expected_peers.append(peer_key)
        #     self.group.peer_map[peer_key] = peer
        #     self.group.expected_peers.sort()  # Sort so all peers have same order

        #     # Once we have all 3 peers, assign round positions
        #     if len(self.group.expected_peers) == 3:
        #         self.my_round = self.group.expected_peers.index(self.my_peer.public_key.key_to_bin()) + 1
        #         print(f"[Peer {self.my_round}] Group ready: {len(self.group.expected_peers)} peers")
        #         self.check_ready_to_send_challenge()
    ...
    def send_nonce(self, peer: Peer, nonce: bytes):
        self.ez_send(peer, NoncePayload(nonce, self.group_id))
        print(f"Sent nonce to peer {peer.public_key.key_to_bin()}")

    def send_signed_nonce(self, peer: Peer, signed_nonce: bytes):
        self.ez_send(peer, SignedNoncePayload(signed_nonce), sig=True)
        print(f"Sent signed nonce to peer {peer.public_key.key_to_bin()}")
    ...
    def send_submission(self, peer: Peer, group_id: str, round_number: int, sig1: bytes, sig2: bytes, sig3: bytes):
        ...
        print(f"Sent submission to peer {peer.public_key.key_to_bin()}")

    def send_confirmation(self, peer: Peer, round_number: int):
        self.ez_send(peer, RoundFinished(round_number))
        print(f"Sent confirmation to peer {peer.public_key.key_to_bin()}")

    def register_group(self):
        self.ez_send(self.server_peer, RegisterPayload(PEER1_PK, PEER2_PK, MY_PEER_PK))
        print(f"Sent registration request to server with peers {PEER1_PK.hex()}, {PEER2_PK.hex()}, {MY_PEER_PK.hex()}")

    @lazy_wrapper(RegisterResponsePayload)
    def on_register_response(self, peer: Peer, payload: RegisterResponsePayload) -> None:
        if peer.public_key.key_to_bin().hex() != SERVER_PUBLIC_KEY.hex():
        ...
        if payload.success:
            ...
            self.ez_send(self.server_peer, ChallengeRequest(self.group_id))
    ...
    def on_challenge_response(self, peer: Peer, payload):
        ...
        self.send_nonce(self.peer1, payload.nonce)
    ...
    def on_receive_nonce(self, peer: Peer, payload):
        ...
        self.send_signed_nonce(peer, payload.nonce)

        print(f"Received nonce from peer {peer.public_key.key_to_bin()} and sent signed nonce back")
    ...
    def on_receive_signed_nonce(self, peer: Peer, payload):
        ...
        print(f"Received signed nonce from peer {peer.public_key.key_to_bin()}")

        if peer == self.peer1:
            self.sig1 = payload.signed_nonce
            if self.sig2 is not None:
                self.send_submission(self.server_peer, self.group_id, self.round, self.sig1, self.my_sig, self.sig2)
    ...
    def on_confirmation(self, peer: Peer, payload):
        ...
        print(f"Received confirmation from peer {peer.public_key.key_to_bin()} for round {payload.round_number}")
    ...
    def started(self) -> None:
        ...
        # Add this peer to its own expected_peers list if it's not the server
        # my_public_key = self.my_peer.public_key.key_to_bin()
        # if my_public_key != SERVER_PUBLIC_KEY:
        # self.group.expected_peers.append(my_public_key)
        # self.group.peer_map[my_public_key] = self.my_peer
        # print(f"[Peer {self.my_round}] Community initialized, waiting for peer discovery...")
...
async def start_client():
    ...
    ipv8 = await IPv8(builder.finalize(), extra_communities={"PoWCommunity": PoWCommunity}).start()
